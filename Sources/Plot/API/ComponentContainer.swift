...
public protocol ComponentContainer: Component {
    ...
    init(@ComponentBuilder content: @escaping ContentProvider) async
}
...
public extension ComponentContainer {
    ...
    init(_ component: Component) async {
        await self.init { component }
    }
    ...
    init(_ string: String) async {
        await self.init { Node<Any>.text(string) }
    }
    ...
    init(html: String) async {
        await self.init { await Node<Any>.raw(html) }
    }
}
